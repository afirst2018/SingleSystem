...
public class QuartzJobGeneral2 implements Job {

    private static final Logger logger = LoggerFactory.getLogger(QuartzJobGeneral2.class);

    @Override
    public void execute(JobExecutionContext context) throws JobExecutionException
    {
        logger.debug("outInf{} " , "独立任务2正在运行start");
        try {
            Thread.sleep(10000);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        logger.debug("outInf{} " , "独立任务2成功运行end");
