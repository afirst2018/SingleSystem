...
/**
 * 扩展自 org.springframework.security.authentication.dao.AbstractUserDetailsAuthenticationProvider
 * 
 * 修改  additionalAuthenticationChecks 方法，使用自定义的 ValidateCodeAuthenticationToken
 * 
 * @author suliang
 *
 */
...
public abstract class AbstractUserDetailsAuthenticationProvider implements AuthenticationProvider, InitializingBean,
	...
	* additional properties of <code>UserDetails</code> and/or <code>ValidateCodeAuthenticationToken</code>,
	* these should also appear in this method.
	*
	* @param userDetails as retrieved from the {@link #retrieveUser(String, ValidateCodeAuthenticationToken)} or
	*		<code>UserCache</code>
	...
	*/
	protected abstract void additionalAuthenticationChecks(UserDetails userDetails,
		ValidateCodeAuthenticationToken authentication)
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		Assert.isInstanceOf(ValidateCodeAuthenticationToken.class, authentication,
			messages.getMessage("AbstractUserDetailsAuthenticationProvider.onlySupports",
				"Only ValidateCodeAuthenticationToken is supported"));
		...
		if (user == null) {
			cacheWasUsed = false;
			try {
				user = retrieveUser(username, (ValidateCodeAuthenticationToken) authentication);
		...
		try {
			...
			additionalAuthenticationChecks(user, (ValidateCodeAuthenticationToken) authentication);
	...
	* ValidateCodeAuthenticationToken)} to compare the credentials of a cached <code>UserDetails</code> with
	...
	* performing it in {@link #additionalAuthenticationChecks(UserDetails, ValidateCodeAuthenticationToken)} so
	...
	*/
	protected abstract UserDetails retrieveUser(String username, ValidateCodeAuthenticationToken authentication)
		throws AuthenticationException;
