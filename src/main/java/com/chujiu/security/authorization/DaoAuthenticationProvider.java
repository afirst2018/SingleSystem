...
/**
 * 扩展自 org.springframework.security.authentication.dao.DaoAuthenticationProvider
 * 增加验证码的处理 使用新的 ValidateCodeAuthenticationToken 替换原有的
...
public class DaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
	...
	protected void additionalAuthenticationChecks(UserDetails userDetails,
			ValidateCodeAuthenticationToken authentication) throws AuthenticationException {
		Object salt = null;
		...
		String validateCode = authentication.getValidateCode();
		if(validateCode == null || "".equals(validateCode)){
			throw new IllegalValidateCodeException("Bad validatecode", userDetails);
		}
		this.captchaService = (CaptchaService)SpringContextUtils.getBeanById(this.captchaServiceId);
		if(this.captchaService == null){
			throw new IllegalValidateCodeException("do validatecode failure", userDetails);
		}
		if ((StringUtils.isEmpty(this.autoPassValue)) || (!this.autoPassValue.equals(validateCode))){
			/* WebAuthenticationDetails 包含了request的相关信息
					  可参照：UsernamePasswordAuthenticationFilter 中的 setDetails方法，查找父抽象类org.springframework.security.web.authentication.AbstractAuthenticationProcessingFilter
					  中的 WebAuthenticationDetailsSource ，其中有对请求request的处理
			 */
			WebAuthenticationDetails details = (WebAuthenticationDetails)authentication.getDetails();
			if (!this.captchaService.validateResponseForID(details.getSessionId(), validateCode).booleanValue()){
				  throw new IllegalValidateCodeException("Bad validatecode", userDetails);
			}
		}
	}
	...
	protected final UserDetails retrieveUser(String username, ValidateCodeAuthenticationToken authentication)
	...
	public String getCaptchaServiceId() {
		return captchaServiceId;
	}

	public void setCaptchaServiceId(String captchaServiceId) {
		this.captchaServiceId = captchaServiceId;
	}
